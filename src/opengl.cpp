...
void Renderer::setup()
{
	...
	m_tonemapProgram = linkProgram({
		compileShader("data/shaders/glsl/tonemap_vs.glsl", GL_VERTEX_SHADER),
	...
	m_pbrModel = createMeshBuffer(Mesh::fromFile("data/model/knife/Knife.fbx"));
	m_pbrProgram = linkProgram({
		compileShader("data/shaders/glsl/pbr_vs.glsl", GL_VERTEX_SHADER),
		compileShader("data/shaders/glsl/pbr_fs.glsl", GL_FRAGMENT_SHADER)
	});

	m_albedoTexture = createTexture(Image::fromFile("data/model/knife/blinn1_Base_Color.png", 3), GL_RGB, GL_SRGB8);
	m_normalTexture = createTexture(Image::fromFile("data/model/knife/blinn1_Normal.png", 3), GL_RGB, GL_RGB8);
	m_metalnessTexture = createTexture(Image::fromFile("data/model/knife/blinn1_Metallic.png", 1), GL_RED, GL_R8);
	m_roughnessTexture = createTexture(Image::fromFile("data/model/knife/blinn1_Roughness.png", 1), GL_RED, GL_R8);
	...
	// Load & convert equirectangular environment map to a cubemap texture.
	{
		GLuint equirectToCubeProgram = linkProgram({
			compileShader("data/shaders/glsl/equirect2cube_cs.glsl", GL_COMPUTE_SHADER)
		});

		Texture envTextureEquirect = createTexture(Image::fromFile("data/HDR_111_Parking_Lot_2_Ref.hdr", 3), GL_RGB, GL_RGB16F, 1);
	...
	// Compute pre-filtered specular environment map.
	{
		GLuint spmapProgram = linkProgram({
			compileShader("data/shaders/glsl/spmap_cs.glsl", GL_COMPUTE_SHADER)
		});
	...
	// Compute diffuse irradiance cubemap.
	{
		GLuint irmapProgram = linkProgram({
			compileShader("data/shaders/glsl/irmap_cs.glsl", GL_COMPUTE_SHADER)
		});
	...
	// Compute Cook-Torrance BRDF 2D LUT for split-sum approximation.
	{
		GLuint spBRDFProgram = linkProgram({
			compileShader("data/shaders/glsl/spbrdf_cs.glsl", GL_COMPUTE_SHADER)
		});
...
{
	const glm::mat4 projectionMatrix = glm::perspectiveFov(view.fov, float(m_framebuffer.width), float(m_framebuffer.height), 1.0f, 10000.0f);
	...
	const glm::mat4 viewMatrix = glm::translate(glm::mat4{ 1.0f }, { 0.0f, -2.0f, -view.distance }) * viewRotationMatrix;
